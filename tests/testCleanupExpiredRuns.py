...
sqlDir = os.path.join(catDir, "sql/mysql")
...
def resetGlobalDb():
    cmd = 'destroyGlobal.py -f %s' % policyF
    subprocess.call(cmd.split())

    cmd = 'setupGlobal.py -f %s' % policyF
    subprocess.call(cmd.split())
...
def createDummyUserAccounts():
    ...
    """
    cmd = 'addMySqlUser.py -f %s -u %s -p %s' % (policyF, u1, p1)
    subprocess.call(cmd.split())
    cmd = 'addMySqlUser.py -f %s -u %s -p %s' % (policyF, u2, p2)
    subprocess.call(cmd.split())
...
""" % (n, n, n, u2))
    ...
    cmd = 'cleanupExpiredRuns.py -f %s -d 2008-05-%i -g %s' % (policyF, n, gDb)
